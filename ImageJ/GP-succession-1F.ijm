...
for(i = 0; i < count; i++){
	...
	for(u = 0; u < stacks; u++){
		...
		rename("stack-" + u);
		stackList[u] = getTitle();

		selectImage(GP);
		close();
	}

	//GP-succession画像を作成する
	stacks = stacks-1;
	
	for(u = 0; u < stacks; u++){
		o = u+1;
		selectWindow(stackList[u]);
		before = getImageID();
		selectWindow(stackList[o]);
		after = getImageID();
		imageCalculator("Substract create 32-bit", before, after);
		rename("succession-" + u);
		selectImage(before);
		close();
	}
	...
	//動画を作成
	run("Images to Stack", "name=GP-movie- + i title=succession- use");
	n = i+1;
