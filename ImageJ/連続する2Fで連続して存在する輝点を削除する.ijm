//連続する2Fで連続して存在する輝点を削除する

prominence = getNumber("Find Maximaで使用する, prominenceの値を入力してください", 10);
judgeDistance = getNumber("フレーム間で何pix移動していれば別の輝点とみなしますか?", 2);
...
for(i = 1; i < sliceRoop; i++) {
	...
	for(n = resultsPointCount; n > 0; n--) {
		...
		//nextの輝点座標を取り出します。
		//取り出した座標とResultの座標が規定以上に近ければ、
		//同一の輝点とみなして、Resultsの表から削除します。
